...
*}  
  ...
  {$exec_build_title="$build_title $title_sep $my_build_name"}
    ...
    {if $cfg->exec_cfg->show_last_exec_any_build && $gui->history_on == 0}
      {if $abs_last_exec.status != '' and 
          ...
          {$status_name=$tlCfg->results.code_status.$status_code}

          {if $status_name == 'failed'}{$status_class='panel-danger'}{/if}
          {if $status_name == 'blocked'}{$status_class='panel-info'}{/if}
          {if $status_name == 'passed'}{$status_class='panel-success'}{/if}
    ...
    <div class="panel {$status_class}">
      ...
      {if $cfg->exec_cfg->show_last_exec_any_build && $gui->history_on == 0}
        {if $abs_last_exec.status != '' and 
            ...
            {$labels.date_time_run} {$title_sep} <div class="label label-default">{localize_timestamp ts=$abs_last_exec.execution_ts}</div>
            {$title_sep_type3}
            {$labels.test_exec_by} {$title_sep} 
            
              {if isset($users[$abs_last_exec.tester_id])}
                <div class="label label-default">{$users[$abs_last_exec.tester_id]->getDisplayName()|escape}</div>
              {else}
            ...
            {$title_sep_type3}
            {$labels.build}{$title_sep} <div class="label label-default">{$abs_last_exec.build_name|escape}</div>
            {$title_sep_type3}
            {$labels.exec_status} {$title_sep} <div class="label label-default">{localize_tc_status s=$status_code}</div>
      ...
      {if $gui->other_execs.$tcversion_id}
        ...
        <table cellspacing="0" class="table table-condensed table-striped table-bordered">
         <tr>
      ...
      {* Initialize panel if notes exists. There might be multiple note panels
        ...
        <tr style="background-color: {$bg_color}">
          ...
          {if $gui->other_exec_cfields[$execID] neq ""}
        <tr style="background-color: {$bg_color}">
    ...
    <div class="panel panel-primary filterable">
        <div class="panel-heading">
            <div class="pull-right">
                <span class="label label-info">{$labels.version}: {$tc_exec.version}</span>
            </div>
            <h3 class="panel-title">
              ...
              [{$gui->tcasePrefix|escape}{$cfg->testcase_cfg->glue_character}{$tc_exec.tc_external_id|escape}] {$ts_name} :: {$tc_exec.name|escape}
            </h3>
        </div>
        <div class="panel-body">
  ...
  <div class="panel panel-primary filterable">
      <div class="panel-heading">{$labels.execute_and_save_results}</div>
      <div class="panel-body">
